...
export default function PortefeuillesPage() {
  const t = useTranslations("Portefeuilles");
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <div className="flex items-center gap-2 text-primary font-bold text-xs">
              <Wallet size={16} /> {t("digitalTitle")}
            </div>
            <p className="text-xl font-extrabold text-base-content mt-2">
              {formatFcfa(numerique?.solde ?? 0)}
            </p>
            <span className="text-[11px] text-muted font-medium">{t("digitalSubtitle")}</span>
          </button>
          ...
          >
            <div className="flex items-center gap-2 text-primary font-bold text-xs">
              <Landmark size={16} /> {t("bankTitle")}
            </div>
            <p className="text-xl font-extrabold text-base-content mt-2">
              {formatFcfa(bancaire?.solde ?? 0)}
            </p>
            <span className="text-[11px] text-muted font-medium">{t("bankSubtitle")}</span>
          </button>
        </div>
        ...
        <section className="card bg-white border border-base-200 p-5 space-y-4 shadow-xs">
          <h2 className="text-h2 flex items-center gap-2">
            {t("historyTitle", { type: onglet === "numerique" ? t("digitalSales") : t("bankPledge") })}
          </h2>

          {!portActif || portActif.mouvements.length === 0 ? (
            <p className="text-sm text-muted text-center py-6">{t("noTransactions")}</p>
          ) : (
