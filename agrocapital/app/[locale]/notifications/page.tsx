    ...
    <PageShell>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-2 bg-base-200 p-1.5 rounded-2xl">
          ...
          >
            {t("tabNotifications")} {totalNonLues > 0 && <span className="badge badge-error badge-sm">{totalNonLues}</span>}
          </button>
          <button
          ...
          >
            {t("tabSubscriptions", { count: suivis.length })}
          </button>
        </div>
        ...
        {onglet === "notifications" ? (
          notifications.length === 0 ? (
            <div className="card bg-white p-8 text-center border border-base-200 space-y-2">
              ...
              <p className="text-sm font-semibold text-muted">{t("noNotifications")}</p>
            </div>
          ) : (
          ...
          <div className="card bg-white p-8 text-center border border-base-200 space-y-2">
            ...
            <p className="text-sm font-semibold text-muted">{t("noSubscriptions")}</p>
          </div>
        ) : (
          <div className="space-y-3">
            {suivis.map((s) => (
              <div key={s.id} className="card bg-white border border-base-200 p-4 flex-row justify-between items-center">
                <div>
                  <h3 className="font-bold text-sm text-base-content">
                    {s.followedUser ? t("followingFarmer", { nom: s.followedUser.nom, region: s.followedUser.region }) : t("followingCrop", { culture: s.cultureSuivie ?? "" })}
                  </h3>
                  <span className="text-xs text-muted">{t("followedSince", { date: dateCourteFr(s.createdAt) })}</span>
                </div>
