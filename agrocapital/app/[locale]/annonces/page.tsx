      ...
      <div className="space-y-6">
        <header className="flex items-center justify-between">
          ...
          {user?.role === "AGRICULTEUR" && (
            <button onClick={() => setCreationModal(true)} className="btn btn-primary btn-sm font-bold">
              <Plus size={16} /> {t("announceHarvest")}
            </button>
          )}
        </header>
        ...
        {loading ? (
          ...
          <div className="card bg-white p-8 text-center border border-base-200 space-y-2">
            ...
            <p className="text-sm font-semibold text-muted">{t("noAnnouncements")}</p>
          </div>
        ) : (
            ...
            {annonces.map((a) => (
              <div key={a.id} className="card bg-white border border-base-200 p-5 space-y-3 shadow-xs">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h3 className="font-bold text-lg text-base-content">
                      {t("harvestExpected", { date: dateCourteFr(a.dateRecoltePrevu) })}
                    </h3>
                    <p className="text-xs text-muted">{t("producer", { nom: a.user.nom, region: a.region })}</p>
                  </div>
                ...
                <div className="flex justify-between items-baseline bg-base-100 p-3 rounded-xl">
                  <div>
                    <span className="text-xs text-muted block">{t("estimatedQty")}</span>
                    <strong className="text-base font-bold">{a.quantiteEstimee} kg</strong>
                  </div>
                  <div className="text-right">
                    <span className="text-xs text-muted block">{t("estimatedPrice")}</span>
                    <strong className="text-base font-bold text-primary">{a.prixEstime ? formatFcfa(a.prixEstime) : t("negotiable")}</strong>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-base-100">
                  <span className="text-xs text-muted font-semibold">
                    {t("reservationsCount", { count: a._count?.reservations ?? 0 })}
                  </span>
                  <button
                    onClick={() => {
                  ...
                  >
                    <BookmarkPlus size={14} /> {t("reservePart")}
                  </button>
                </div>
              </div>
            ))}
        ...
        {creationModal && (
          <dialog className="modal modal-open">
            <div className="modal-box bg-white p-6 space-y-4">
              <h3 className="font-bold text-lg">{t("createModalTitle")}</h3>
              <form onSubmit={handleCreerAnnonce} className="space-y-3">
                <div className="form-control gap-1">
                  <label htmlFor="a-cult" className="label-text text-xs font-bold">{t("cropLabel")}</label>
                  <select
                    id="a-cult"
                    value={culture}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="form-control gap-1">
                    <label htmlFor="a-qte" className="label-text text-xs font-bold">{t("estimatedQtyKg")}</label>
                    <input
                      id="a-qte"
                      type="number"
                  ...
                  <div className="form-control gap-1">
                    <label htmlFor="a-prix" className="label-text text-xs font-bold">{t("estimatedPriceFcfa")}</label>
                    <input
                      id="a-prix"
                      type="number"
                      value={prixEstime}
                ...
                <div className="form-control gap-1">
                  <label htmlFor="a-date" className="label-text text-xs font-bold flex items-center gap-1">
                    <Calendar size={14} /> {t("harvestDate")}
                  </label>
                  <input
                    id="a-date"
                    type="date"
                ...
                <div className="form-control gap-1">
                  <label htmlFor="a-reg" className="label-text text-xs font-bold">{t("regionLabel")}</label>
                  <select
                    id="a-reg"
                    value={region}
                ...
                <div className="modal-action flex gap-2">
                  ...
                  <button type="submit" disabled={creerState} className="btn btn-primary flex-1 font-bold">
                    {creerState ? <span className="loading loading-spinner" /> : t("publishAnnounce")}
                  </button>
                </div>
              </form>
            </div>
        ...
        {annonceSelectionnee && (
          <dialog className="modal modal-open">
            <div className="modal-box bg-white p-6 space-y-4">
              <h3 className="font-bold text-lg">{t("reserveModalTitle", { culture: annonceSelectionnee.culture })}</h3>

              {succesRes ? (
                <div className="text-center py-6 space-y-3">
                  <CheckCircle2 size={48} className="text-primary mx-auto" />
                  <h4 className="text-h2">{t("reserveSuccessTitle")}</h4>
                  <p className="text-sm text-muted">
                    {t("reserveSuccessText", { nom: annonceSelectionnee.user.nom })}
                  </p>
                ...
                <form onSubmit={handleReserver} className="space-y-3">
                  <div className="form-control gap-1">
                    <label htmlFor="res-qte" className="label-text text-xs font-bold">{t("desiredQtyKg")}</label>
                    <input
                      id="res-qte"
                      type="number"
                      min={10}
                  ...
                  <div className="form-control gap-1">
                    <label htmlFor="res-comm" className="label-text text-xs font-bold">{t("commentLabel")}</label>
                    <textarea
                      id="res-comm"
                  ...
                  <div className="modal-action flex gap-2">
                    ...
                    <button type="submit" disabled={reserverState} className="btn btn-primary flex-1 font-bold">
                      {reserverState ? <span className="loading loading-spinner" /> : t("confirmReservation")}
                    </button>
                  </div>
                </form>
              )}
