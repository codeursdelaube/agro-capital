  ...
  const handlePasserCommande = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const json = await res.json();
        setErreurCommande(json.error ?? t("orderError"));
      }
    } catch {
      setErreurCommande(t("networkError"));
    } finally {
          ...
          <dialog className="modal modal-open">
            <div className="modal-box bg-white p-6 space-y-4">
              <h3 className="font-bold text-lg text-base-content">
                {t("orderModalTitle", { nom: produitSelectionne.nom })}
              </h3>

              {commandeSucces ? (
                <div className="text-center py-6 space-y-3">
                  <CheckCircle2 size={48} className="text-primary mx-auto" />
                  <h4 className="text-h2">{t("orderSuccessTitle")}</h4>
                  <p className="text-sm text-muted">
                    {t("orderSuccessText", { total: formatFcfa(produitSelectionne.prixUnitaire * quantiteCommande) })}
                  </p>
                  <button
                ...
                <form onSubmit={handlePasserCommande} className="space-y-4">
                  ...
                  <div className="form-control gap-1">
                    <label htmlFor="qte-cmd" className="label-text font-bold">{t("qtyLabel", { unite: produitSelectionne.uniteMesure })}</label>
                    <input
                      id="qte-cmd"
                      type="number"
                      min={1}
                  ...
                  <div className="form-control gap-1">
                    <label htmlFor="adr-cmd" className="label-text font-bold">{t("addressLabel")}</label>
                    <input
                      id="adr-cmd"
                      type="text"
                  ...
                  <div className="modal-action flex gap-2">
                    ...
                    >
                      {commandeEnCours ? <span className="loading loading-spinner" /> : t("confirmOrder")}
                    </button>
                  </div>
                </form>
              )}
