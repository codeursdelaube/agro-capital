    ...
    <PageShell>
      <div className="space-y-6">
        <header className="flex items-center justify-between">
          <div>
            <p className="text-eyebrow">Marketplace Agro-Capital</p>
            <h1 className="mt-1 text-h1">Boutiques Agricoles</h1>
          </div>
        </header>
        ...
        {user?.role === "AGRICULTEUR" && (
          <section className="card bg-primary/5 border border-primary/20 p-5 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-h2">Ma Vitrine Numérique</h2>
                <p className="text-xs text-muted">Espace de vente directe sur la marketplace</p>
              </div>
            </div>
            ...
            {maBoutique ? (
              <div className="bg-white p-4 rounded-xl border border-base-200 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-muted">{maBoutique.description ?? "Aucune description"}</p>
                  </div>
              ...
              <form onSubmit={handleCreate} className="bg-white p-4 rounded-xl border border-base-200 space-y-3">
                <p className="text-sm font-semibold">Vous n&apos;avez pas encore créé votre boutique.</p>
                {erreur && <div className="alert alert-error text-xs p-2">{erreur}</div>}
                <div className="form-control gap-1">
                  <label htmlFor="b-nom" className="label-text text-xs font-bold">Nom de votre boutique</label>
                  <input
                    id="b-nom"
                    type="text"
                    placeholder="Ex: Ferme Bio Akouvi & Fils"
                    value={nom}
                ...
                <div className="form-control gap-1">
                  <label htmlFor="b-desc" className="label-text text-xs font-bold">Description</label>
                  <textarea
                    id="b-desc"
                    placeholder="Présentez vos produits et votre village..."
                    value={description}
                ...
                <button type="submit" disabled={creerState} className="btn btn-primary btn-sm w-full">
                  {creerState ? <span className="loading loading-spinner" /> : <><Plus size={16} /> Créer ma boutique</>}
                </button>
              </form>
            )}
          </section>
        ...
        <section>
          <h2 className="text-h2 mb-3">Toutes les boutiques de la région</h2>
              ...
              <div key={b.id} className="card bg-white border border-base-200 p-4 space-y-2 hover:shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted">Propriétaire : {b.user.nom} ({b.user.region})</p>
                  </div>
                </div>
                {b.description && <p className="text-xs text-muted line-clamp-2">{b.description}</p>}
                <div className="flex justify-between items-center pt-2 text-xs">
                  <span className="text-primary font-semibold">{b._count?.produits ?? 0} produit(s) en catalogue</span>
